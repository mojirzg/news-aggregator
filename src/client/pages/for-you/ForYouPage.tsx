...
export const ForYouPage = () => {
  ...
  const allProvidersFailed = Boolean(
    feed.data?.providers.length &&
    feed.data.providers.every((provider) => provider.status === 'error'),
  );
  const editPreferences = () => {
    void navigate(routes.preferences);
  };
      ...
      <div className={`container ${styles.content}`}>
        <header className={styles.header}>
          <div className={styles.copy}>
            ...
            <p className={styles.subtitle}>
              Your saved source, category, and author preferences are converted
              into normalized feed filters.
            </p>
          </div>
          <Button type="button" variant="secondary" onClick={editPreferences}>
            Edit preferences
          </Button>
        </header>
        ...
        {!configured ? (
          <EmptyState
            ...
            action={
              <Button type="button" onClick={editPreferences}>
                Choose preferences
              </Button>
            }
          />
        ...
        {configured && (feed.isError || allProvidersFailed) ? (
          <ArticleFeedError onRetry={() => void feed.refetch()} />
        ) : null}
        ...
        !allProvidersFailed &&
        feed.data.articles.length > 0 ? (
          <ArticleFeed articles={feed.data.articles} />
        ) : null}
      </div>
    </main>
  );
