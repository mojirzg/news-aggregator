...
export const AuthorPreferences = ({ value, onChange }: { value: FeedPreferences; onChange: (next: FeedPreferences) => void }) => (
  <Input
    ...
    onChange={(event) => onChange({
      ...value,
      authors: event.target.value.split(',').map((author) => author.trim()).filter(Boolean),
    })}
  />
);
