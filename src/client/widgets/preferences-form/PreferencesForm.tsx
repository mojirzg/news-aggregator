...
import { usePreferencesForm, validatePreferences } from '@client/features/edit-feed-preferences';
...
export const PreferencesForm = ({ initial, onSave, onReset }: {
  ...
  return (
    <form className={styles.form} onSubmit={(event: FormEvent<HTMLFormElement>) => { event.preventDefault(); submit(); }}>
      <section className={styles.section}>
        <h2 className={styles.title}>Preferred sources</h2>
        <p className={styles.description}>Leave all unchecked to include every configured provider.</p>
        <SourcePreferences value={form.draft} onChange={form.setDraft} />
      </section>
      <section className={styles.section}>
        <h2 className={styles.title}>Preferred categories</h2>
        <p className={styles.description}>Select the topics that should shape your personalized feed.</p>
      ...
      <div className={styles.actions}>
        {form.saved ? <span className={styles.saved} role="status">Preferences saved</span> : null}
        <Button type="button" variant="secondary" onClick={() => { onReset(); form.setDraft({ schemaVersion: 1, sourceIds: [], categories: [], authors: [] }); }}>Reset</Button>
