...
export const ProviderFailureDetails = ({ providers }: { providers: ProviderResult[] }) => {
  const failedNames = providers
    .filter((provider) => provider.status === 'error')
    .map((provider) => providerOptions.find((option) => option.id === provider.providerId)?.label ?? provider.providerId);
