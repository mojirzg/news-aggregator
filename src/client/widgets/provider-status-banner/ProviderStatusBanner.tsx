...
export const ProviderStatusBanner = ({ providers, onRetry }: { providers: ProviderResult[]; onRetry: () => void }) => {
  ...
  return (
    <aside className={styles.banner} role="status">
      <div>
        <div className={styles.title}>Showing results from {succeeded} of {providers.length} sources</div>
        <div className={styles.text}><ProviderFailureDetails providers={providers} /> Successful articles remain visible.</div>
      </div>
      <Button type="button" size="small" variant="secondary" onClick={onRetry}>Retry</Button>
    </aside>
  );
};
